...
public class VesselTests implements TestSetup
{
    @Test
    ...
    @Test
    public void testVesselProvidesItemFluidAndHeatCapabilities()
    {
        final ItemStack stack = new ItemStack(TFCItems.VESSEL);
    ...
    private void checkTimeToHeat(ItemStack stack, int expectedTicks)
    {
        ...
        try (CalendarTransaction tr = Calendars.get().transaction())
        {
            ...
            while (heat.getTemperature() < targetTemperature)
            {
                ...
                tr.add(1);
                ticks++;
                assertNotEquals(ticks, 1_000_000, "Loop did not terminate with stack " + stack);
            }

            assertEquals(expectedTicks, ticks, "Expected " + expectedTicks + " to heat " + stack + " to " + targetTemperature + "C, got " + ticks);
        }
    }
