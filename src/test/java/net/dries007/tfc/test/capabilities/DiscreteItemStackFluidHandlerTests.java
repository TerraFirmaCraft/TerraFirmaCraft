...
package net.dries007.tfc.test.capabilities;
...
public class DiscreteItemStackFluidHandlerTests extends TestHelper
{
    ...
    @Test
    public void testFillLessThanCapacity()
    {
        final ItemStackFluidHandler handler = create();

        assertEquals(0, handler.fill(new FluidStack(Fluids.WATER, 123), IFluidHandler.FluidAction.EXECUTE));
        assertEquals(FluidStack.EMPTY, handler.getFluidInTank());
    }
    ...
    public void testFillWithNotAllowedFluid()
    {
        ...
        assertEquals(0, handler.fill(new FluidStack(Fluids.LAVA, 1000), IFluidHandler.FluidAction.EXECUTE));
        assertEquals(FluidStack.EMPTY, handler.getFluidInTank());
    }
    ...
    public void testDrain()
    {
        ...
        assertEquals(new FluidStack(Fluids.WATER, 1000), handler.drain(1000, IFluidHandler.FluidAction.SIMULATE));
        assertEquals(new FluidStack(Fluids.WATER, 1000), handler.getFluidInTank());
        assertEquals(new FluidStack(Fluids.WATER, 1000), handler.drain(1000, IFluidHandler.FluidAction.EXECUTE));
        assertEquals(FluidStack.EMPTY, handler.getFluidInTank());
    }

    @Test
    public void testDrainLessThanCapacity()
    {
        final ItemStackFluidHandler handler = createFilled();

        assertEquals(FluidStack.EMPTY, handler.drain(123, IFluidHandler.FluidAction.EXECUTE));
        assertEquals(new FluidStack(Fluids.WATER, 1000), handler.getFluidInTank());
    }
    ...
    public void testDrainIncorrectContent()
    {
        ...
        assertEquals(FluidStack.EMPTY, handler.drain(new FluidStack(Fluids.LAVA, 1000), IFluidHandler.FluidAction.EXECUTE));
        assertEquals(new FluidStack(Fluids.WATER, 1000), handler.getFluidInTank());
    }
    ...
    private DiscreteItemStackFluidHandler createFilled()
    {
        final DiscreteItemStackFluidHandler handler = create();
        handler.fill(new FluidStack(Fluids.WATER, 1000), IFluidHandler.FluidAction.EXECUTE);
        return handler;
    }

    private DiscreteItemStackFluidHandler create()
    {
        return new DiscreteItemStackFluidHandler(new ItemStack(Items.APPLE), fluid -> fluid == Fluids.WATER, 1000);
    }
}
