...
public class JunkEventHandler
{
    //TODO: Dirty recipe removal, hope it be better in 1.13/1.14. Both ways to remove recipes put crap in the log.
    // Advancements for TFC are pointless anyway so I opted for this.
    // For whatever reason it only works here, It can't be part of Common and requires it to be registered in preint. #NotMyJobToCare
    @SubscribeEvent
    public void removeRecipe(RegistryEvent.Register<IRecipe> event)
    {
        if (ConfigTFC.GENERAL.removeVanillaRecipes)
        {
            ...
            //Comment out for a recipe name dump.
//            for (Object r : modRegistry.getKeys())
//            {
//                TerraFirmaCraft.getLog().info(r);
//            }
