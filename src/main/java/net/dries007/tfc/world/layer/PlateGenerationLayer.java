...
public class PlateGenerationLayer implements ITypedAreaTransformer0<Plate>
{
    ...
    public Plate apply(ITypedNoiseRandom<Plate> context, int x, int z)
    {
        ...
        context.pickRandom(Float.floatToRawIntBits(centerX), Float.floatToRawIntBits(centerZ));
        for (int j = 0; j < 10; j++) context.random(1);
        boolean oceanic = context.random(100) < oceanPercent;
        float angle = 2 * PI * context.random(100) / 100f;
        float velocity = context.random(100) / 100f;
        float elevation = context.random(100) / 100f;
