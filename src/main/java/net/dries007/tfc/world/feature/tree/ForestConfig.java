...
public record ForestConfig(List<Entry> entries) implements FeatureConfiguration
{
    ...
    public record Entry(float minRainfall, float maxRainfall, float minAverageTemp, float maxAverageTemp, Optional<BlockState> bushLog, Optional<BlockState> bushLeaves, BlockState fallenLog, Optional<List<BlockState>> groundcover, Supplier<ConfiguredFeature<?, ?>> treeFeature, Optional<Supplier<ConfiguredFeature<?, ?>>> oldGrowthFeature)
    {
        public static final Codec<Entry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codecs.BLOCK_STATE.fieldOf("fallen_log").forGetter(c -> c.fallenLog),
