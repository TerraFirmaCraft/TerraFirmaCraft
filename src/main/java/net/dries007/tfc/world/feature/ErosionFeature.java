...
public class ErosionFeature extends Feature<NoFeatureConfig>
{
    ...
    public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, NoFeatureConfig config)
    {
        ...
        BlockRecipeWrapper.Mutable mutableWrapper = new BlockRecipeWrapper.Mutable(worldIn.getWorld());
        ...
        for (int x = 0; x < 16; x++)
        {
            for (int z = 0; z < 16; z++)
            {
                for (int y = worldIn.getHeight(Heightmap.Type.WORLD_SURFACE_WG, chunkX + x, chunkZ + z); y >= 0; y--)
                {
                    ...
                    if (LandslideRecipe.getRecipe(worldIn.getWorld(), mutableWrapper).map(recipe -> {
                ...
                if (resultPos != null && !resultPos.equals(landslidePos))
                {
                    ...
                    if (stateAt.getBlock() instanceof TFCGrassBlock)
                    {
                        ...
                        if (pastDirtState.getBlock() instanceof TFCDirtBlock)
                        {
                            ...
                            worldIn.setBlockState(mutablePos, dirtBlock.getGrass().getDefaultState(), 2);
                        ...
                        if (pastGrassState.getBlock() instanceof TFCGrassBlock)
                        {
                            ...
                            worldIn.setBlockState(mutablePos, grassBlock.getDirt().getDefaultState(), 2);
                        }
                    }
                }
            }
