...
/**
 * todo: fix cascading lag. Priority: medium - low. See <a href=
 * "https://github.com/TerraFirmaCraft/TerraFirmaCraft/issues/40">issue</a>
 * here.
 */
public class WorldGenFissure implements IWorldGenerator
{
	...
	protected void generate(World world, Random rng, BlockPos start)
	{
		...
		if (BlocksTFC.isWater(block) && !BlocksTFC.isGround(block))
			return;
		...
		for (int d = 1; d <= poolDepth; d++)
			if (!world.getBlockState(start.add(0, -d, 0)).isNormalCube())
				return;
		...
		for (BlockPos pos : list)
		{
			...
			for (int d = 0; d <= creviceDepth; d++)
			{
				...
				if (rng.nextInt(3) == 0)
					carve(world, pos.add(-1 + rng.nextInt(3), d, -1 + rng.nextInt(3)), rock);
			}
		...
		if (list.size() > 10)
			makeTunnel(rng, world, start.add(0, -poolDepth - 1, 0), rock, localFillBlock);
	}
	...
	private List<BlockPos> getCollapseMap(World world, BlockPos pos, IBlockState fillBlock, int poolDepth)
	{
		...
		final IBlockState rock = fillBlock == ChunkGenTFC.LAVA ? BlockRockVariant.get(getRock3(world, pos), Rock.Type.RAW).getDefaultState() : BlockRockVariant.get(ChunkDataTFC.getRockHeight(world, pos), Rock.Type.RAW).getDefaultState();
	...
	{
		// todo: check if this should even update the blocks (flags = 3 means update) I
		// think this only causes lag. (if not also replace `setBlockToAir`)
		if (!world.isAirBlock(pos) && BlocksTFC.isGround(world.getBlockState(pos)))
			world.setBlockToAir(pos);
		BlockPos p = pos.add(-1, 0, 0);
		if (!world.isAirBlock(p) && BlocksTFC.isGround(world.getBlockState(p)))
	...
	private void fill(World world, BlockPos pos, IBlockState rock, IBlockState fillBlock)
	{
		...
		{
			if (facing == EnumFacing.UP || world.getBlockState(pos.offset(facing)) == fillBlock)
				continue;
	...
	private void makeTunnel(Random random, World world, BlockPos pos, IBlockState rock, IBlockState fillBlock)
	{
		...
		while (pos.getZ() > minTunnel)
		{
			...
			BlockPos p = pos.add(1, 0, 0);
			if (world.getBlockState(p).getMaterial() != fillBlock.getMaterial())
				world.setBlockState(p, rock, 2);
			p = pos.add(-1, 0, 0);
			if (world.getBlockState(p).getMaterial() != fillBlock.getMaterial())
				world.setBlockState(p, rock, 2);
			p = pos.add(0, 0, 1);
