...
public class WorldGenOreVeins implements IWorldGenerator
{
	...
	// Gets veins at a single chunk. Deterministic for a specific chunk x/z and
	// world seed
	@Nonnull
	private static List<Vein> getVeinsAtChunk(int chunkX, int chunkZ, Long worldSeed)
	{
		...
		for (int i = 0; i < NUM_ROLLS; i++)
		{
			...
			// Avoid uninitialized entries in case the config empty
			if (entries == null)
			{
				continue;
			}
			if (rand.nextDouble() < entries.getTotalWeight())
			{
				...
				BlockPos startPos = new BlockPos(chunkX * 16 + 8 + rand.nextInt(16), veinType.minY + rand.nextInt(veinType.maxY - veinType.minY), chunkZ * 16 + 8 + rand.nextInt(16));
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider)
	{
		...
		for (Vein vein : veins)
		{
			...
			if (!vein.type.baseRocks.contains(chunkData.getRock1(0, 0)) && !vein.type.baseRocks.contains(chunkData.getRock2(0, 0)) && !vein.type.baseRocks.contains(chunkData.getRock3(0, 0)))
				continue;
			...
			for (int x = 0; x < 16; x++)
			{
				for (int z = 0; z < 16; z++)
				{
					// Do checks here that are specific to the the horizontal position, not the
					// vertical one
					if (!vein.inRange(new BlockPos(xoff + x, 0, zoff + z)))
						continue;
