		...
		switch (plant.getPlantType())
		{
			case MUSHROOM:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 16; ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && plantBlock.canPlaceBlockAt(worldIn, blockpos))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case SHORT_GRASS:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 4; ++i)
				{
					...
					if (plant.isValidGrowthTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && plantBlock.canBlockStay(worldIn, blockpos, state))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case TALL_GRASS:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 16; ++i)
				{
					...
					for (int k = 0; k < j; ++k)
					{
						if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos.up(k))) && worldIn.isAirBlock(blockpos.up(k)) && plantBlock.canBlockStay(worldIn, blockpos.up(k), state))
						{
							int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case CREEPING:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 16; ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && plantBlock.canBlockStay(worldIn, blockpos, state) && !BlocksTFC.isSand(worldIn.getBlockState(blockpos.down())))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case HANGING:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 4; ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && plantBlock.canBlockStay(worldIn, blockpos, state))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case REED_SEA:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 16; ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && worldIn.getBlockState(blockpos.down()).getBlock().canSustainPlant(state, worldIn, blockpos.down(), EnumFacing.UP, plantBlock))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
						setBlockAndNotifyAdequately(worldIn, blockpos, state.withProperty(BlockPlantTFC.AGE, plantAge));
					}
			...
			case DESERT:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position); ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && !BiomeDictionary.hasType(worldIn.getBiome(blockpos), BiomeDictionary.Type.BEACH) && plantBlock.canBlockStay(worldIn, blockpos, state))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
						setBlockAndNotifyAdequately(worldIn, blockpos, state.withProperty(BlockPlantTFC.AGE, plantAge));
					}
			...
			case WATER_SEA:
			{
				...
				if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && plantBlock.canPlaceBlockAt(worldIn, blockpos))
				{
					int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case FLOATING:
			{
				...
				for (int i = 0; i < 8; ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.isAirBlock(blockpos) && plantBlock.canPlaceBlockAt(worldIn, blockpos) && plant.isValidFloatingWaterDepth(worldIn, blockpos, water))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
			...
			case EPIPHYTE:
			{
				...
				for (int i = 0; i < ChunkDataTFC.getRainfall(worldIn, position) / 4; ++i)
				{
					...
					if (plant.isValidTemp(ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos)) && plant.isValidSunlight(worldIn.getLightFor(EnumSkyBlock.SKY, blockpos)) && worldIn.getBlockState(blockpos).getBlock().isReplaceable(worldIn, blockpos) && plantBlock.canPlaceBlockAt(worldIn, blockpos))
					{
						int plantAge = plant.getAgeForWorldgen(rand, ClimateTFC.getHeightAdjustedTemp(worldIn, blockpos));
