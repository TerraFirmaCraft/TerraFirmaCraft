...
/**
 * todo: make these bigger without causing cascading lag. This will require
 * larger re-writes on the scale of oregen Wait for 1.13+ as AlcatrazEscapee is
 * doing a worldgen rewrite anyway
 */
...
//                    cranberryGen.generate(world, random, pos);
	...
	{
		// If this has to have a radius that is >= 8, then it needs to be moved to a
		// cascading-lag safe model
		// Otherwise, do not change this unless you are prepared to do some fairly large
		// re-writes, similar to how ore gen is handled
		int radius = rng.nextInt(6) + 2;
		int depth = rng.nextInt(3) + 1;
		if (rng.nextInt(CLAY_CHUNK_RARITY) != 0 || start.getY() > WorldTypeTFC.SEALEVEL + 6)
			return;
		if (ChunkDataTFC.getRainfall(world, start) < CLAY_RAINFALL_THREHOLD)
			return;
				...
				if (flag && rng.nextInt(15) == 0)
				{
					...
					for (Plant plant : TFCRegistries.PLANTS.getValuesCollection())
					{
						if (plant.getIsClayMarking())
						{
							...
							if (!world.provider.isNether() && !world.isOutsideBuildHeight(pos) && plant.isValidLocation(ClimateTFC.getHeightAdjustedTemp(world, pos), ChunkDataTFC.getRainfall(world, pos), world.getLightFor(EnumSkyBlock.SKY, pos)) && world.isAirBlock(pos) && plantBlock.canBlockStay(world, pos, state))
							{
	...
	private boolean generatePeat(World world, Random rng, BlockPos start)
	{
		...
		if (rng.nextInt(30) != 0 || start.getY() > WorldTypeTFC.SEALEVEL)
			return false;
		...
		for (int x = -radius; x <= radius; ++x)
		{
			for (int z = -radius; z <= radius; ++z)
			{
				...
				for (int y = -depth; y <= depth; ++y)
				{
					...
					if (!ClimateTFC.isSwamp(world, pos))
						continue;
					final IBlockState current = world.getBlockState(pos);
