...
public final class BiomesTFC
{
	...
	public static void registerBiomes(RegistryEvent.Register<Biome> event)
	{
		...
		register(r, new BiomeTFC(new Biome.BiomeProperties(MOD_NAME + " Mountians").setBaseHeight(-1.9000001f).setHeightVariation(-1.1f)).setSpawnBiome(), true, true, BiomeDictionary.Type.MOUNTAIN);
		register(r, new BiomeTFC(new Biome.BiomeProperties(MOD_NAME + " Mountians Edge").setBaseHeight(-2.3f).setHeightVariation(-1.9000001f).setBaseBiome("tfc:mountians")), false, false, BiomeDictionary.Type.MOUNTAIN);
	...
	private static void register(IForgeRegistry<Biome> r, Biome biome, boolean isSpawn, boolean isWorldGen, BiomeDictionary.Type... types)
	{
		...
		// These need to happen after the biomes are constructed, otherwise they will be
		// null
		if (isSpawn)
		{
