...
public class VeinType
{
	...
	public VeinType(@Nullable Ore ore, Size size, Shape shape, Collection<Rock> baseRocks, int rarity, int minY, int maxY, int density)
	{
		...
		this.minY = minY;
		this.maxY = maxY;
		this.density = 0.01D * (double) density; // For debug purposes, removing the 0.01D will lead to ore veins being full
													// size, easy to see shapes
	}

	/**
	 * Intended to be used by propick via
	 * {@code VeinRegistry.INSTANCE.getVeins().values().forEach(x -> x.ore.isOreBlock(state)}
	...
	public enum Size
	{
		SMALL(8.0F, 0.7F), MEDIUM(12.0F, 0.6F), LARGE(16.0F, 0.5F);
