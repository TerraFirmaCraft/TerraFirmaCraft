...
public final class ClimateTFC
{
	...
	static
	{
		// internationally accepted average lapse time is 6.49 K / 1000 m, for the first
		// 11 km of the atmosphere. I suggest graphing our temperature
		// across the 110 m against 2750 m, so that gives us a change of 1.6225 / 10
		// blocks, which isn't /terrible/
		// Now going to attempt exponential growth. calculations but change in
		// temperature at 17.8475 for our system, so that should be the drop at 255.
		// therefore, change should be temp - f(x), where f(x) is an exp function
		// roughly equal to f(x) = (x^2)/ 677.966.
		// This seems to work nicely. I like this. Since creative allows players to
		// travel above 255, I'll see if I can't code in the rest of it.
		...
		{
			// temp = temp - (ySq / 677.966f) * (((110.01f - y) + Math.abs(110.01f - y)) /
			// (2 * (110.01f - y)));
			// temp -= (0.16225 * y * (((y - 110.01f) + Math.abs(y - 110.01f)) / (2 * (y -
			// 110.01f))));
	...
	public static float getHeightAdjustedTemp(World world, BlockPos pos)
	{
		...
		if (temp <= 0 || !world.canBlockSeeSky(pos))
			return temp;
	 ...
	 * @return The month adjusted temperature. This gets the base temperature,
	 *         before daily / hourly changes
	 */
	 ...
	 *
	 * @param regionalTemp The base temperature, either from {@link ChunkDataTFC} or
	 *                     {@link ClimateRenderHelper}
	...
	private static float getTemp(float regionalTemp, int z)
	{
		...
		// Note: this does not use world seed, as that is not synced from server -
		// client, resulting in the seed being different
