...
//        if (chunkData == null) throw new IllegalStateException("ChunkData capability is missing.");
		...
		/*
		 * fissue gen, is commented out in 1.7.10 if
		 * (this.rand.nextInt(chunkData.isStable(0, 0) ? 4 : 6) == 0) { x = xCoord +
		 * this.rand.nextInt(16) + 8; z = zCoord + this.rand.nextInt(16) + 8; y =
		 * Global.SEALEVEL - rand.nextInt(45); }
		 */
...
//        net.minecraftforge.event.ForgeEventFactory.onChunkPopulate(false, this, world, rand, chunkX, chunkZ, false); //todo
	...
	private boolean canSnowAt(BlockPos pos)
	{
		if (!world.isAirBlock(pos) && !world.isAirBlock(pos.add(0, -1, 0)) && !SNOW.getBlock().canPlaceBlockAt(world, pos))
			return false;
		if (ClimateTFC.getHeightAdjustedTemp(world, pos) >= 0F)
			return false;
	...
	private void generateHeightMap(int xPos, int zPos)
	{
		...
		for (int x = 0; x < 5; ++x)
		{
			for (int z = 0; z < 5; ++z)
			{
				...
				if (scaledNoise6Value < 0.0D) // Only true when noise6[index2] is between -17,777 and 0, scaledNoise6Value
												// will be at maximum -2
				{
	...
	private void replaceBlocksForBiomeHigh(int chunkX, int chunkZ, ChunkPrimer inp, CustomChunkPrimer outp)
	{
		...
		for (int x = 0; x < 16; ++x)
		{
			for (int z = 0; z < 16; ++z)
			{
				...
				{
					if (!BiomesTFC.isBeachBiome(getBiomeOffset(x, z)))
				...
				for (int y = 127; y >= 0; y--)
				{
					...
					if (outp.isEmpty(x, y + yOffset, z))
					{
						...
						if (y + 1 < yOffset && outp.getBlockState(x, y + yOffset, z) == AIR/*
																							 * no need to check again && BlocksTFC.isSoilOrGravel(outp.getBlockState(x, y +
																							 * yOffset + 1, z))
																							 */)
						{
					...
					if (outp.getBlockState(x, y + yOffset, z) == STONE)
					{
						...
						if (smooth == -1)
						{
							// The following makes dirt behave nicer and more smoothly, instead of forming
							// sharp cliffs.
							...
							{
								if (arrayIndexx >= 0 && seaLevelOffsetMap[colIndex] - (3 * counter) > seaLevelOffsetMap[arrayIndexx] && arrayIndexX >= 0 && seaLevelOffsetMap[colIndex] - (3 * counter) > seaLevelOffsetMap[arrayIndexX] && arrayIndexz >= 0 && seaLevelOffsetMap[colIndex] - (3 * counter) > seaLevelOffsetMap[arrayIndexz] && arrayIndexZ >= 0 && seaLevelOffsetMap[colIndex] - (3 * counter) > seaLevelOffsetMap[arrayIndexZ])
								{
					...
					// && biome != BiomesTFC.OCEAN && biome != BiomesTFC.DEEP_OCEAN && biome !=
					// BiomesTFC.BEACH && biome != BiomesTFC.GRAVEL_BEACH
				...
				for (int y = 127; y >= 0; y--) // This cannot be optimized with the prev for loop, because the sealeveloffset
												// won't be ready yet.
				{
					/*
