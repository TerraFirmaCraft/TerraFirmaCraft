...
{
    PERIODIC_X((seed, scale) -> new SinNoise(1, 0, 1f / scale, 0).extendY().add(new SimplexNoise2D(seed).octaves(2).spread(12f / scale).scaled(-0.2f, 0.2f))),
    PERIODIC_Z((seed, scale) -> new SinNoise(1, 0, 1f / scale, 0).extendX().add(new SimplexNoise2D(seed).octaves(2).spread(12f / scale).scaled(-0.2f, 0.2f))),
    GRADIENT_X((seed, scale) -> PERIODIC_X.create(seed, scale).warped((x, z) -> MathHelper.clamp(x, -scale / 2f, scale / 2f), (x, z) -> z)),
    GRADIENT_Z((seed, scale) -> PERIODIC_Z.create(seed, scale).warped((x, z) -> x, (x, z) -> MathHelper.clamp(x, -scale / 2f, scale / 2f))),
