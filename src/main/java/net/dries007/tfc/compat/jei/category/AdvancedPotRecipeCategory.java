...
import net.dries007.tfc.common.recipes.AdvancedPotRecipe;
...
public class AdvancedPotRecipeCategory extends PotRecipeCategory<PotRecipe>
{
    ...
    public AdvancedPotRecipeCategory(RecipeType<PotRecipe> type, IGuiHelper helper)
    {
    ...
    {
        AdvancedPotRecipe recipe = (AdvancedPotRecipe) potRecipe;
        ...
        {
            final List<ItemStack> stack = collapse(List.of(ingredients.get(j).getItems()), provider);
            if (!stack.isEmpty())
            {
                IRecipeSlotBuilder output = builder.addSlot(RecipeIngredientRole.OUTPUT, OUTPUT_X[j] + 1, OUTPUT_Y[j] + 1);
                output.addItemStacks(stack);
