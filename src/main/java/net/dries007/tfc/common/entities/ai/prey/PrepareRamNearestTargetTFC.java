...
public class PrepareRamNearestTargetTFC<E extends PathfinderMob> extends Behavior<E>
{
    ...
    private Optional<BlockPos> calculateRammingStartPosition(PathfinderMob rammingPrey, LivingEntity target) {
        ...
        if (!this.isWalkableBlock(rammingPrey, blockpos)) {
            ...
            return list.stream().sorted(Comparator.comparingDouble(rammingPrey.blockPosition()::distSqr)).filter((p_147753_) -> {
    ...
    private void chooseRamPosition(PathfinderMob rammingPrey, LivingEntity target) {
        ...
        this.ramCandidate = this.calculateRammingStartPosition(rammingPrey, target).map((p_289369_) -> {
            return new net.minecraft.world.entity.ai.behavior.PrepareRamNearestTarget.RamCandidate(p_289369_, target.blockPosition(), target);
        });
    }
}
