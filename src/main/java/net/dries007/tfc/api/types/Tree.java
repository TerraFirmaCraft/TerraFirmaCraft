...
public class Tree extends IForgeRegistryEntry.Impl<Tree>
{
	...
	/**
	 * This is a registry object that will create a number of things: 1. Wood logs,
	 * planks, and leaf blocks, and all the respective variants 2. A Tree object to
	 * be used in TFC world gen
	 *
	 * Addon mods that want to add trees should subscribe to the registry event for
	 * this class They also must put (in their mod) the required resources in
	 * /assets/tfc/...
	 *
	 * When using this class, use the provided Builder to create your trees. This
	 * will require all the default values, as well as provide optional values that
	 * you can change
	 *
	 * @param name             the ResourceLocation registry name of this tree
	 * @param gen              the generator that should be called to generate this
	 *                         tree, both during world gen and when growing from a
	 *                         sapling
	 * @param minTemp          min temperature
	 * @param maxTemp          max temperature
	 * @param minRain          min rainfall
	 * @param maxRain          max rainfall
	 * @param minDensity       min density. Use -1 to get all density values. 0.1 is
	 *                         the default, to create really low density regions of
	 *                         no trees
	 * @param maxDensity       max density. Use 2 to get all density values
	 * @param dominance        how much this tree is chosen over other trees. Range
	 *                         0 <> 10 with 10 being the most common
	 ...
	 * @param minGrowthTime    the amount of time (in in-game days) that this tree
	 *                         requires to grow
	 * @param burnTemp         the temperature at which this will burn in a fire pit
	 *                         or similar
	 * @param burnTicks        the number of ticks that this will burn in a fire pit
	 *                         or similar
	 */
