...
/**
 * Capability for any food item Allows foods to have nutrients, and also to
 * decay / rot
 */
public interface IFood extends INBTSerializable<NBTTagCompound>
{
	...
	/**
	 * Sets the creation date Use to apply preservation over time DO NOT TRY AND
	 * PRESERVE ALREADY ROTTEN FOOD Example: - A ceramic large vessel will tick
	 * randomly. Between each tick, it tracks a tick counter. - On a tick, for each
	 * item in inventory, it will try and "preserve" the item based on it being in
	 * the vessel for the time between the last random tick and the current tick -
	 * It then would call setCreationDate(getCreationDate() +
	 * ticksSinceLastRandomTick * decayModifier) - A decay modifier of 0 is no
	 * preservation, a decay modifier = 1 is 100% preservation (i.e. never decaying)
	 *
	...
	/**
	 * How much thirst is restored when this item is eaten. Drinking water from a
	 * water source gives 15, for reference
	 *
	...
	/**
	 * Gets the current list of traits on this food Can also be used to add traits
	 * to the food
	 *
	...
	/**
	 * Tooltip added to the food item Called from
	 * {@link net.dries007.tfc.client.ClientEvents}
	 *
	...
	default void addNutrientInfo(@Nonnull ItemStack stack, @Nonnull List<String> text)
	{
		...
		// next level shows which nutrients it has, i.e. "Category: Grain, Nutrients:
		// Carbohydrates, Fat"
		// final level shows exact values, i.e. "Category: Grain, Nutrients:
		// Carbohydrates (1.5), Fat (3.0)"
