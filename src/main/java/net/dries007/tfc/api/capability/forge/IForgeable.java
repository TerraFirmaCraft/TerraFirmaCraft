...
/**
 * This is an advanced IItemHeat capability that is used by items that can be
 * forged If you implement this capability, you MUST implement
 * {@link net.dries007.tfc.api.capability.heat.CapabilityItemHeat} as well You
 * should return the same instance from the getCapability calls
 */
public interface IForgeable extends IItemHeat
{
	...
	/**
	 * Gets the current saved recipe's registry name Returns null if no recipe name
	 * is currently saved
	 */
	@Nullable
	...
	/**
	 * Sets the recipe name from an {@link AnvilRecipe}. If null, sets the recipe
	 * name to null
	 */
	...
	/**
	 * Gets the last three steps, wrapped in a {@link ForgeSteps} instance. The
	 * return value is nonnull, however the individual steps might be
	 */
	@Nonnull
	ForgeSteps getSteps();
	 ...
	 *
	 * @param step The step to add. In general this should not be null, although it
	 *             is perfectly valid for it to be
	 */
	void addStep(@Nullable ForgeStep step);

	/**
	 * Resets the object's {@link IForgeable} components. Used if an item falls out
	 * of an anvil without getting worked Purpose is to preserve stackability on
