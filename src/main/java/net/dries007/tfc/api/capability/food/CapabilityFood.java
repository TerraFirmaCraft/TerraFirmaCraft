...
public class CapabilityFood
{
	 ...
	 * That puts decay times at 25% - 100% of this value So meat / fruit will decay
	 * in ~4 days, grains take ~16 days Other modifiers are applied on top of that
	 */
	...
	public static final IFoodTrait.Impl LARGE_VESSEL_PRESERVED = new IFoodTrait.Impl("preserved", 0.5f);
	...
	public static void preInit()
	{
		...
		TRAITS.put("salted", SALTED); // todo: In 1.7.10 this was 0.5 for uncooked meat, 0.75 for cooked. Requires a
										// custom class.
		TRAITS.put("pickled", PICKLED); // todo: same as above
		TRAITS.put("preserved", LARGE_VESSEL_PRESERVED);
	}
	...
	/**
	 * Helper method to handle applying a trait to a food item. Do NOT just directly
	 * apply the trait, as that can lead to strange interactions with decay dates /
	 * creation dates This calculates a creation date that interpolates between no
	 * preservation (if the food is rotten), to full preservation (if the food is
	 * new) Trust me, this works. (Thank CtrlAltDavid for figuring it out)
	 */
	...
	/**
	 * Helper method to handle removing a trait to a food item. Do NOT just directly
	 * remove the trait, as that can lead to strange interactions with decay dates /
	 * creation dates
	 */
	public static void removeTrait(IFood instance, IFoodTrait trait)
	{
		if (instance.getTraits().contains(trait))
		{
			...
			{
				// If not rotten, re-calculate the creation date as to respect remaining decay
				// time
				instance.setCreationDate(CalendarTFC.PLAYER_TIME.getTicks() - (long) ((CalendarTFC.PLAYER_TIME.getTicks() - instance.getCreationDate()) * CapabilityFood.LARGE_VESSEL_PRESERVED.getDecayModifier()));
			}
		}
