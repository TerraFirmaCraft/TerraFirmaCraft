...
public interface ITreeGenerator
{
	...
	/**
	 * Called to generate a tree. Each Tree must have one of these. Used for world
	 * gen and sapling growth
	 *
	 * @param manager an instance of the world's template manager. Used for getting
	 *                structures.
	...
	/**
	 * Checks if a tree can be generated. This implementation checks height, radius,
	 * and light level
	 *
	 * @param world    The world
	...
	default boolean canGenerateTree(World world, BlockPos pos, Tree treeType)
	{
		...
		for (int x = -radius; x <= radius; x++)
		{
			...
			{
				if ((x == 0 && z == 0) || world.getBlockState(pos.add(x, 0, z)).getMaterial().isReplaceable() || ((x > 1 || z > 1) && world.getBlockState(pos.add(x, 1, z)).getMaterial().isReplaceable()))
					continue;
				return false;
			}
		}
