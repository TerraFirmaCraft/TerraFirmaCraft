...
public class TEBarrel extends TEInventory implements ITickable, IItemHandlerSidedCallback, IFluidHandlerSidedCallback, IFluidTankCallback
{
	 ...
	 * Loads its data from the Item's NBTTagCompound without loading xyz
	 * coordinates.
	 *
	...
	/**
	 * Called to get the NBTTagCompound that is put on Barrel Items. This happens
	 * when a sealed Barrel was broken.
	 *
	...
	/**
	 * Called once per side when the TileEntity has finished loading. On servers,
	 * this is the earliest point in time to safely access the TE's World object.
	 */
	@Override
	public void onLoad()
	{
	...
	/**
	 * Retrieves the packet to send to clients whenever this TileEntity is updated
	 * via World.notifyBlockUpdate. We are using this method to update the lock
	 * status on our ItemHandler and FluidHandler, since a Block update occurred.
	...
	/**
	 * Called on clients whenever this TileEntity received an update from the
	 * server.
	 **/
	@Override
	...
	/**
	 * Called on clients when this TileEntity received an update from the server on
	 * load.
	 *
