...
public class TECrucible extends TEInventory implements ITickable, ITileFields
{
	...
	public void update()
	{
		...
		{
			// Crucible target temperature decays constantly, since it is set by outside
			// providers
		...
		if (cap instanceof IMoldHandler)
		{
			...
			if (lastFillTimer <= 0)
			{
				if (mold.isMolten())
				{
					// At this point we assume that ((FluidMetal) fluidStack.getFluid()).getMetal()
					// == mold.getMetal()
					// Anything that breaks this contract is a bug and should be reported
					// This also happens here to avoid off-by-one errors as it will report null
					// after the mold has been emptied
	...
	/**
	 * Used on CLIENT to update the alloy contents Also updates cached alloy result
	 *
	...
	/**
	 * Used on CLIENT for quicker rendering - doesn't have to calculate the alloy
	 * every render tick
	 *
