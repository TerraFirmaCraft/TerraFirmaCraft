...
public class TEBloomery extends TEInventory implements ITickable
{
	...
	public BlockPos getInternalBlock()
	{
		...
		{
			internalBlock = pos.up(OFFSET_INTERNAL.getY()).offset(direction, OFFSET_INTERNAL.getX()).offset(direction.rotateY(), OFFSET_INTERNAL.getZ());
		}
	...
	public BlockPos getExternalBlock()
	{
		...
		{
			externalBlock = pos.up(OFFSET_EXTERNAL.getY()).offset(direction, OFFSET_EXTERNAL.getX()).offset(direction.rotateY(), OFFSET_EXTERNAL.getZ());
		}
	...
	private void updateSlagBlock(boolean cooking)
	{
		...
		// If there's at least one item, show one layer so player knows that it is
		// holding stacks
