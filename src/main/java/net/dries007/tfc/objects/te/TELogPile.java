...
public class TELogPile extends TEInventory implements ITickable
{
	...
	// This function does some magic **** to not create floating charcoal. Don't
	// touch unless broken
	// - AlcatrazEscapee
	private void createCharcoal()
	{
		...
		do
		{
			...
			// Because the pile scans Top > Bottom this is necessary to avoid floating
			// blocks
		...
		// If you exit the loop, its arrived back at the original position OR needs to
		// rest the original position, and needs to replace that block
		world.setBlockState(pos, BlocksTFC.CHARCOAL_PILE.getDefaultState().withProperty(LAYERS, charcoal));
	}
}
