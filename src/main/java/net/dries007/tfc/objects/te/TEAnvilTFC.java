...
public class TEAnvilTFC extends TEInventory
{
	...
	/**
	 * Sets the anvil TE recipe, called after pressing the recipe button This is the
	 * ONLY place that should write to {@link this#recipe}
	 *
	...
	public void addStep(@Nullable ForgeStep step)
	{
		...
		if (cap != null)
		{
			...
			if (step != null)
			{
				...
				// The line below should be changed to a "HIT" sound, not 3 hits(minecraft
				// default)
	...
	public boolean attemptWelding(EntityPlayer player)
	{
		...
		{
			// No message as this will happen whenever the player clicks the anvil with a
			// hammer
			return false;
		}
		...
		if (recipe != null)
		{
			...
			if (heatResult != null)
			{
				// Every welding result should have this capability, but don't fail if it
				// doesn't
