...
/**
 * This is an ingredient wrapper for various types It includes static
 * constructors for both item stack and fluid stack ingredients
 *
...
public interface IIngredient<T> extends Predicate<T>
{
	...
	/**
	 * This is used by recipes to test if an input is valid, but necessarily high
	 * enough quantity i.e. used to check if an item is valid for a slot, not if the
	 * recipe will complete.
	 *
	 * @param input the input supplied to the recipe
	 * @return true if the ingredient matches the input, ignoring the amount of
	 *         input
	 */
	...
	/**
	 * Consume one recipe's worth of input Depending on the input type, this may
	 * modify input and return it, or create a new output
	 *
	 * @param input the input supplied to the recipe
