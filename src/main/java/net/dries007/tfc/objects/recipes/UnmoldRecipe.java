			...
			if (!stack.isEmpty())
			{
				if (stack.getItem() instanceof ItemMold)
				{
					...
					if (Constants.RNG.nextFloat() <= chance)
					{
						...
						{
							// This can't use the remaining items, because vanilla doesn't sync them on
							// crafting, thus it gives a desync error
							// To fix: ContainerWorkbench#onCraftMatrixChanged needs to call
