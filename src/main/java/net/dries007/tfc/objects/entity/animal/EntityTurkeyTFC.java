...
public class EntityTurkeyTFC extends EntityAnimalTFC implements ILivestock
{
    //The last time(in ticks) this chicken has laid eggs
    private static final DataParameter<Long> LAID = EntityDataManager.createKey(EntityTurkeyTFC.class, Helpers.LONG_DATA_SERIALIZER);
    ...
    public List<ItemStack> getProducts()
    {
        ...
        if (this.isFertilized())
        {
            ...
            if (cap != null)
            {
                EntityTurkeyTFC chick = new EntityTurkeyTFC(this.world);
                chick.setFamiliarity(this.getFamiliarity() < 0.9F ? this.getFamiliarity() / 2.0F : this.getFamiliarity() * 0.9F);
                cap.setFertilized(chick, ConfigTFC.Animals.TURKEY.hatch + CalendarTFC.PLAYER_TIME.getTotalDays());
            }
        }
        eggs.add(egg);
        return eggs;
    ...
    {
        return Math.max(0, ConfigTFC.Animals.TURKEY.eggTicks + getLaidTicks() - CalendarTFC.PLAYER_TIME.getTicks());
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.getClass() == EntityTurkeyTFC.class && this.getGender() == Gender.MALE && !this.world.isRemote && !this.isChild() && CalendarTFC.CALENDAR_TIME.getHourOfDay() == 6 && CalendarTFC.CALENDAR_TIME.getMinuteOfHour() == 0)
        {
