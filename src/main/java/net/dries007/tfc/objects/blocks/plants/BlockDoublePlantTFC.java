...
public class BlockDoublePlantTFC extends BlockStackPlantTFC implements IGrowable
{
    ...
    public void grow(World worldIn, Random rand, BlockPos pos, IBlockState state)
    {
        ...
        IBlockState iblockstate = state.withProperty(AGE, 0).withProperty(GROWTH_STAGE, CalenderTFC.getMonthOfYear().id()).withProperty(PART, state.getValue(PART));
    ...
    {
        return state.withProperty(TIME_OF_DAY, state.getValue(TIME_OF_DAY)).withProperty(AGE, state.getValue(AGE)).withProperty(GROWTH_STAGE, CalenderTFC.getMonthOfYear().id()).withProperty(PART, getPlantPart(worldIn, pos));
    }
    ...
    {
        return this.getDefaultState().withProperty(AGE, meta).withProperty(GROWTH_STAGE, CalenderTFC.getMonthOfYear().id());
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        if ((ClimateTFC.getHeightAdjustedBiomeTemp(worldIn, pos) < 15 || !plant.isValidSunlight(worldIn.getLightFromNeighbors(pos.up()))) && canShrink(worldIn, pos, state, worldIn.isRemote))
        {
            ...
            if (rand.nextFloat() < getGrowthRate(worldIn, pos) && net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, pos, state, true))
            {
                ...
                {
                    worldIn.setBlockState(pos, state.withProperty(AGE, j - 1).withProperty(GROWTH_STAGE, state.getValue(GROWTH_STAGE)).withProperty(PART, state.getValue(PART)));
                }
