...
public class BlockSupport extends Block
{
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	{
		...
		if (axis == EnumFacing.Axis.Y)
		{
			...
			if (!isConnectable(worldIn, pos, EnumFacing.DOWN) && !placer.isSneaking() && stack.getCount() > 2 // Need 3 or more because at this point itemstack didn't shrink for the first
																												// block
		...
		} else
		{
			...
			{
				// Another vertical support to connect not found or player don't have enough
				// items to place.
				worldIn.destroyBlock(pos, true);
			} else if (distance > 0)
			{
	...
	/**
	 * Checks if this support block can support collapsable/fallable blocks Returns
	 * true only if this is horizontally placed and can stay in place.
	 *
	...
	{
		// If above or below is support, or below is solid, this one must be vertical
		// too
		...
		return (isConnectable(world, pos, EnumFacing.NORTH) || isConnectable(world, pos, EnumFacing.SOUTH)) && (isConnectable(world, pos, EnumFacing.EAST) || isConnectable(world, pos, EnumFacing.WEST));
	}
	...
	private EnumFacing.Axis getAxis(IBlockAccess world, BlockPos pos)
	{
		...
		// If none of the above, this is a new placement without any neighboring support
		// blocks
		return EnumFacing.Axis.Y;
	}
	 ...
	 * @return 0 if not found, 1-5 block distance between this BlockPos and the
	 *         found vertical support
	 */
