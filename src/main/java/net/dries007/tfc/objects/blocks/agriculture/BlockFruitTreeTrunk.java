...
public class BlockFruitTreeTrunk extends Block
{
	...
	{
		// Helper method, gets a missing leaf block position that connects to this
		// branch
		...
		List<BlockPos> positions = Arrays.asList(branchPos.offset(branchFacing.rotateY()), branchPos.offset(branchFacing.rotateY().getOpposite()), branchPos.offset(branchFacing).offset(branchFacing.rotateY()), branchPos.offset(branchFacing).offset(branchFacing.rotateY().getOpposite()));
	...
	{
		// Helper method, checks if all 5 leafs blocks(horizontals + above) has a
		// missing block leaf
