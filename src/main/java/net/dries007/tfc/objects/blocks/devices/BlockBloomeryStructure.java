...
public class BlockBloomeryStructure extends Block
{
    public static final PropertyBool LIT = PropertyBool.create("lit");
    public static final PropertyBool FIRED = PropertyBool.create("fired");

    public BlockBloomeryStructure()
    {
        super(Material.IRON);
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos)
    {
        ...
        BlockPos bottom = new BlockPos(pos.getX(), pos.getY(), pos.getZ());
        int structPos = 0; //Where in the structure height am I? 0 = bottom
        while(te == null)
        {
            structPos++;
            //Check for bottom block until a TE is returned
            bottom = bottom.add(0,-1,0);
            te = Helpers.getTE(source, bottom, TEBloomery.class);
            if(structPos > 3)break;
        }
        double y;
        if( te == null){
            y = 1d;
        }else if (structPos < te.getHeight()){
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rng)
    {
        ...
        //Check if top block is Air
        if (world.getBlockState(pos.add(0,1,0)).getMaterial() != Material.AIR)return;
        if (rng.nextInt(24) == 0)
        {
        ...
        if (!worldIn.isRemote)
        {
            if (!state.getValue(LIT))
            {
                ...
                while(te == null)
                {
                    ...
                    //Check for bottom block until a TE is returned
                    bottom = bottom.add(0,-1,0);
                    te = Helpers.getTE(worldIn, bottom, TEBloomery.class);
                    if(structPos > 3)break;
                }
