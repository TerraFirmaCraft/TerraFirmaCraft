...
public final class BlocksTFC
{
	...
	public static void registerBlocks(RegistryEvent.Register<Block> event)
	{
...

		{
			...
			// Full slabs are the same as full blocks, they are not saved to a list, they
			// are kept track of by the halfslab version.
			for (Rock.Type type : new Rock.Type[] { SMOOTH, COBBLE, BRICKS })
				for (Rock rock : TFCRegistries.ROCKS.getValuesCollection())
			...
			// Anvils are special because they don't have an ItemBlock + they only exist for
			// certian types
			for (Rock rock : TFCRegistries.ROCKS.getValuesCollection())
		...
		// Note: if you add blocks you don't need to put them in this list of todos.
		// Feel free to add them where they make sense :)
		// todo: pumpkin/melon ?
		...
		// todo: smoke rack (placed with any string, so event based?) + smoke blocks or
		// will we use particles?
		// todo: custom flower pot (TE based probably, unless we want to not care about
		// the dirt in it)

		// todo: custom hopper or just a separate press block? I prefer the separate
		// block, this will simplify things a lot.

		allNormalItemBlocks = normalItemBlocks.build();
		allInventoryItemBlocks = inventoryItemBlocks.build();

		// Register Tile Entities
		// Putting tile entity registration in the respective block can call it multiple
		// times. Just put here to avoid duplicates
	...
	public static boolean isHotWater(IBlockState current)
	{
		return current == FLUID_HOT_WATER.getDefaultState();
	}
