...
public class BlockRockVariantFallable extends BlockRockVariant implements IFallingBlock
{
	...
	public BlockPos getFallablePos(World world, BlockPos pos)
	{
		...
		if (type.canFallHorizontal())
		{
			...
			EnumFacing[] faces = Arrays.stream(EnumFacing.HORIZONTALS).filter(x -> world.getBlockState(pos.offset(x)).isNormalCube()).toArray(EnumFacing[]::new);
			...
			faces = Arrays.stream(EnumFacing.HORIZONTALS).filter(x -> shouldFall(world, pos.offset(x), pos) && canFallThrough(world.getBlockState(pos.offset(x)))).toArray(EnumFacing[]::new);
	...
	{
		// Check that there are no entities in the area, otherwise it would collide with
		// them
		if (!world.getEntitiesWithinAABB(EntityFallingBlockTFC.class, new AxisAlignedBB(pos, pos.add(1, 1, 1))).isEmpty())
		{
			// If we can't fall due to a collision, wait for the block to move out of the
			// way and try again later
