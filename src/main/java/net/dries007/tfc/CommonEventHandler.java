...
public final class CommonEventHandler
{
	...
	/**
	 * Handler for {@link IPlaceableItem} To add a new placeable item effect, either
	 * implement {@link IPlaceableItem} or see {@link IPlaceableItem.Impl} for
	 * vanilla item usages Notes: 1) `onBlockActivate` doesn't get called when the
	 * player is sneaking, unless doesSneakBypassUse returns true. 2) This event
	 * handler is fired first with the main hand as event.getStack() If nothing
	 * happens (i.e. the event is not cancelled + set cancellation result to success
	...
	/**
	 * This is an extra handler for items that also have an active effect when right
	 * clicked in the air Note: If you have an item that needs an active effect, use
	 * onItemRightClick(), or attach this via {@link IPlaceableItem.Impl}
	 */
	@SubscribeEvent
	...
	public static void attachItemCapabilities(AttachCapabilitiesEvent<ItemStack> event)
	{
		...
		{
			boolean canStack = stack.getMaxStackSize() > 1; // This is necessary so it isn't accidentally overridden by a default
															// implementation
	...
	public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event)
	{
		if (event.player instanceof EntityPlayerMP)
		{
			...
			if (!(originalStats instanceof FoodStatsTFC))
			{
				...
				// Also need to read the food stats from nbt, as they were not present when the
				// player was loaded
	...
	/**
	 * Fired on server only when a player dies and respawns, or is cloned via other
	 * means (?)
	 *
