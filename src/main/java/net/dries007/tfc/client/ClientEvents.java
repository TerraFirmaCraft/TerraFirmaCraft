...
public class ClientEvents
{
	...
	public static void onInitGuiPre(GuiScreenEvent.InitGuiEvent.Pre event)
	{
		if (ConfigTFC.CLIENT.makeWorldTypeClassicDefault && event.getGui() instanceof GuiCreateWorld)
		{
			...
			// Only change if default is selected, because coming back from customisation,
	...
	public static void onGuiButtonPress(GuiScreenEvent.ActionPerformedEvent.Post event)
	{
		...
		{
			// This should generally be true, but check just in case something has disabled
			// it
		...
		{
			// This is necessary to catch the resizing of the inventory gui when you open
			// the recipe book
	...
	public static void onRenderGameOverlayText(RenderGameOverlayEvent.Text event)
	{
		...
		if (ConfigTFC.GENERAL.debug && mc.gameSettings.showDebugInfo)
		{
			...
			if (mc.world.isBlockLoaded(blockpos) && !chunk.isEmpty())
			{
				...
				{
					list.add(String.format("%sRegion: %s%.1f\u00b0C%s Avg: %s%.1f\u00b0C%s Min: %s%.1f\u00b0C%s Max: %s%.1f\u00b0C", GRAY, WHITE, data.getRegionalTemp(), GRAY, WHITE, data.getAverageTemp(), GRAY, WHITE, ClimateTFC.monthTemp(data.getRegionalTemp(), Month.JANUARY, blockpos.getZ()), GRAY, WHITE, ClimateTFC.monthTemp(data.getRegionalTemp(), Month.JULY, blockpos.getZ())));
					list.add(String.format("%sTemperature: %s%.1f\u00b0C Daily: %s%.1f\u00b0C", GRAY, WHITE, ClimateRenderHelper.get(blockpos).getTemperature(), WHITE, ClimateTFC.getHeightAdjustedTemp(mc.world, blockpos)));
					...
					// list.add(GRAY + "Rocks: " + WHITE + data.getRockLayer1(x, z).name + ", " +
					// data.getRockLayer2(x, z).name + ", " + data.getRockLayer3(x, z).name);
					...
					// list.add("");
					// list.add(GRAY + "Rock at feet: " + WHITE + data.getRockLayerHeight(x,
					// blockpos.getY(), z).name);
