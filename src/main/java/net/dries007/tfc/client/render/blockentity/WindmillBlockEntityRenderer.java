...
public class WindmillBlockEntityRenderer implements BlockEntityRenderer<WindmillBlockEntity>
{
    ...
    public static final Map<Item, Provider<Function<BlockEntityRendererProvider.Context, WindmillBladeModel>>> BLADE_MODELS = Util.make(new IdentityHashMap<>(), map -> {
        ...
        map.put(TFCItems.WINDMILL_BLADE.get(), new Provider<>(defaultTexture, DyeColor.WHITE, defaultModel));
        TFCItems.COLORED_WINDMILL_BLADES.forEach((color, item) -> map.put(item.get(), new Provider<>(defaultTexture, color, defaultModel)));
        map.put(TFCItems.LATTICE_WINDMILL_BLADE.get(), new Provider<>(
            ...
            DyeColor.WHITE,
            context -> new WindmillBladeLatticeModel(context.bakeLayer(RenderHelpers.modelIdentifier("windmill_blade_lattice")))
        ));
        map.put(TFCItems.RUSTIC_WINDMILL_BLADE.get(), new Provider<>(
            Helpers.identifier("textures/entity/misc/windmill_blade_rustic.png"),
            DyeColor.WHITE,
            context -> new WindmillBladeRusticModel(context.bakeLayer(RenderHelpers.modelIdentifier("windmill_blade_rustic")))
        ));
    });
    ...
    private static Function<BlockEntityRendererProvider.Context, WindmillBladeModel> defaultModelFactory()
    {
        ...
        return context -> {
            ...
            if (cache.getValue() == null) cache.setValue(Pair.of(context, new WindmillBladeModel(context.bakeLayer(RenderHelpers.modelIdentifier("windmill_blade")))));
    ...
    public WindmillBlockEntityRenderer(BlockEntityRendererProvider.Context context)
    {
        ...
        this.fallbackModel = Objects.requireNonNull(bladeModels.get(TFCItems.WINDMILL_BLADE.get())); // Fallback to default windmill blade
    }
    ...
    public void render(WindmillBlockEntity windmill, float partialTick, PoseStack stack, MultiBufferSource bufferSource, int packedLight, int packedOverlay)
    {
        ...
        for (int i = 0; i < bladeCount; i++)
        {
            ...
            final float[] color = provider.color == DyeColor.WHITE ? NO_COLOR : provider.color.getTextureDiffuseColors();
