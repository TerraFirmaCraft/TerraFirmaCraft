...
public class TripHammerBlockEntityRenderer implements BlockEntityRenderer<TripHammerBlockEntity>
{
    ...
    public void render(TripHammerBlockEntity hammer, float partialTick, PoseStack stack, MultiBufferSource buffers, int packedLight, int packedOverlay)
    {
        ...
        if (level == null || rotation == null || !(state.getBlock() instanceof TripHammerBlock))
            return;
        ...
        if (item.isEmpty() || !(item.getItem() instanceof HammerItem hammerItem))
            return;
        final ResourceLocation hammerTexture = hammerItem.getMetalTexture();
        ...
        final float angle = hammer.getRealRotationDegrees(rotation, partialTick);
