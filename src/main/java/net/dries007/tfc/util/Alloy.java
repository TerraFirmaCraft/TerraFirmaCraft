...
public class Alloy implements INBTSerializable<NBTTagCompound>
{
	...
	/**
	 * Constructs an alloy with a maximum limit. Anything added over this limit will
	 * do nothing
	 *
	...
	/**
	 * Add metal to an alloy from each item in an inventory Note if the an item
	 * doesn't implement {@link IMetalObject} it will be ignored, and
	 * {@param isValid} will be set to false
	 *
	...
	/**
	 * Adds metal to an alloy from a fluid stack Note if the fluid is not an
	 * instance of {@link net.dries007.tfc.objects.fluids.FluidMetal} then it will
	 * be ignored, and {@link Alloy#isValid} will be set to false
	 *
	...
	/**
	 * Add metal to an alloy from an item stack Note if the an item doesn't
	 * implement {@link IMetalObject} it will be ignored, and {@link Alloy#isValid}
	...
	/**
	 * Removes an amount of the current result from the alloy Use
	 * {@link Alloy#getResult()} to see what alloy has been removed / returned
	 *
	 * @param removeAmount the amount to remove
	 ...
	 *
	 * @return is the alloy valid (set if it was constructed via ItemStacks and one
	 *         ItemStack wasn't an IMetalObject)
	 */
	...
	private boolean matchesRecipeExact(AlloyRecipe recipe)
	{
		...
		{
			if (!metalMap.containsKey(metal) || !recipe.getMetals().containsKey(metal) || !recipe.getMetals().get(metal).test(metalMap.get(metal) / totalAmount))
				return false;
		}
		return true;
	}
