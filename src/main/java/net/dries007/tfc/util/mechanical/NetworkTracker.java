...
public final class NetworkTracker
{
    ...
    public static void updateAllNetworkComponents(MechanicalNetwork network)
    {
        ...
        final GraphResult result = populateGraph(source, source.levelOrThrow(), source.getBlockPos());
        if (result.valid)
        {
    ...
    public static GraphResult populateGraph(IRotator current, Level level, BlockPos pos)
    {
        final List<IRotator> list = new LinkedList<>();
        ...
        for (IRotator neighbor : getConnections(current, level, pos))
        {
            ...
            {
                GraphResult graphResult = populateGraph(neighbor, neighbor.levelOrThrow(), neighbor.getBlockPos());
                if (!graphResult.valid)
                {
                    return new GraphResult(List.of(), false);
                }
                list.addAll(graphResult.rotators);
            }
        }
        return new GraphResult(list, true);
    }
