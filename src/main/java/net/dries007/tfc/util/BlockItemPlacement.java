...
/**
 * This is a fake {  BlockItem} copy pasta for a vanilla item that we want to behave like a block item for a specific block.
 */
public class BlockItemPlacement implements InteractionManager.OnItemUseAction
{
    /**
     * Copy pasta from { BlockItem#updateState(BlockState, Property, String)}
     */
    private static <T extends Comparable<T>> BlockState updateState(BlockState state, Property<T> property, String value)
    {
        return property.parseValue(value).map(valueIn -> state.with(property, valueIn)).orElse(state);
    }
    ...
    /**
     * Copy paste from { ItemStack#useOn(ItemUseContext)}
     */
    @Override
    ...
    /**
     * Copy pasta from { net.minecraft.item.BlockItem#place(BlockItemUseContext)}
     */
        ...
        else
        {
            ...
            else
            {
                ...
                if (placedBlock == placementState.getBlock())
                {
                    ...
                    placedBlock.onBlockPlacedBy(world, pos, placedState, player, stack);
    ...
    protected boolean canPlace(BlockItemUseContext context, BlockState stateToPlace)
    {
        ...
        return (stateToPlace.canEntitySpawn(context.getWorld(), context.getPos(), EntityType.PLAYER)) && context.getWorld().isBlockModifiable(player,context.getPos());//.isUnobstructed(stateToPlace, context.getPos(), selectionContext);
    }

    /**
     * Copy pasta from {  BlockItem#updateBlockStateFromTag(BlockPos, World, ItemStack, BlockState)}
     */
    ...
    private BlockState updateBlockStateFromTag(BlockPos pos, World world, ItemStack stack, BlockState state)
    {
        ...
        if (nbt != null)
        {
            ...
            for (String propertyKey : blockStateNbt.keySet())
            {
                Property<?> property = container.getProperty(propertyKey);
                if (property != null)
                {
                    String s1 = blockStateNbt.get(propertyKey).getString();
