...
public class SimpleCraftMatrix
{
	...
	/**
	 * Create a patterned matrix based on a string pattern input. See
	 * {@link net.dries007.tfc.types.DefaultRecipes} for usage of how to construct
	 * recipes
	 *
	 * @param outsideSlotRequired If the recipe is smaller than MAX_WIDTH x
	 *                            MAX_HEIGHT, what is the slot outside of the recipe
	 *                            required to be? true = outside slots need to be
	 *                            full false = outside slots need to be empty
	 * @param pattern             A list of strings. Each string is a row, each
	 *                            character is an element. ' ' represents empty,
	 ...
	 * @return if the matrices are identical. Not used for checking if recipe
	 *         matches
	 */
	 ...
	 *
	 * @param other Another craft matrix
	 * @return if 'other' is a subset of the current craft matrix (i.e. other is
	 *         found somewhere within the current matrix)
	 */
	...
	private boolean matches(SimpleCraftMatrix other, int startX, int startY, boolean isMirrored)
	{
		for (int x = 0; x < this.width; x++)
		{
			for (int y = 0; y < this.height; y++)
			{
				...
				{
					// If the current position in the matrix is outside the pattern, the value
					// should be set by other.outsideSlot
