...
public class CalendarTFC implements INBTSerializable<NBTTagCompound>
{
	...
	/*
	 * The offset in ticks between the world time (sun position) and the calendar
	 * (since world time 0 = 6 AM)
	 */
	...
	/**
	 * Calendar time. Advances when player sleeps, stops when doDaylightCycle is
	 * false Synced with the daylight cycle Players can see this via the calendar
	 * GUI tab
	 */
	...
	/**
	 * Sets the per-world month length. Will also recalculate all time that has
	 * passed so far
	 *
	...
	/**
	 * Sets the player time by resetting the offset. This should only be used for
	 * debugging purposes
	 *
	...
	public void setPlayerTime(World world, long playerTime)
	{
		...
		{
			// Don't set the player time directly, instead set the offset from the total
			// time
	...
	/**
	 * Sets the calendar time. This actually sets the calendar offset - i.e. the
	 * difference between the current world time and the calendar time It then will
	 * save that offset to world data, and sync to client
	 *
	 * @param world        The world
	...
	public void setCalendarTime(World world, long calendarTime)
	{
		...
		// Don't set the calendar time directly, instead set the offset from the total
		// time
