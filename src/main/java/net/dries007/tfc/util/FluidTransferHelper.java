...
{
	/**
	 * Tries to empty a fluid container item into the fluid handler, then stores the
	 * remainder in the given inventory. If the fluid can't be transferred into the
	 * tank or the remainder can't fit into the return inventory, the action will be
	 * aborted.
	 *
	 * @param container        The filled fluid container ItemStack to empty. Will
	 *                         not be modified directly, if modifications are
	 *                         necessary a modified copy is returned in the result.
	 * @param fluidDestination The fluid destination to fill from the fluid
	 *                         container.
	 * @param returnInventory  An inventory where empty containers are put.
	 * @param returnSlot       The slot id into which the empty containers are
	 *                         inserted.
	 ...
	 *         container. The resulting container is empty on failure.
	 */
	@Nonnull
	...
	/**
	 * Tries to fill a fluid container item from the fluid handler, then stores the
	 * result in the given inventory. If the fluid can't be transferred into the
	 * container or the result can't fit into the return inventory, the action will
	 * be aborted.
	 *
	 * @param container       The fluid container ItemStack to fill. Will not be
	 *                        modified directly, if modifications are necessary a
	 *                        modified copy is returned in the result.
	 * @param fluidSource     The fluid source to fill from.
	 * @param returnInventory An inventory where filled containers are put.
	 * @param returnSlot      The slot id into which the filled containers are
	 *                        inserted.
	 ...
	 *
	 * @param container        The filled container. Will not be modified. Separate
	 *                         handling must be done to reduce the stack size, stow
	 *                         containers, etc, on success.
	 * @param fluidDestination The fluid handler to be filled by the container.
	 * @param maxAmount        The largest amount of fluid that should be
	 *                         transferred.
	 * @param doDrain          True if the container should actually be drained,
	 *                         false if it should be simulated.
	 * @param world            World to play the sound in. Null for no sound.
	 * @param pos              BlockPos to play the sound at. Null for no sound.
	 * @return A {@link FluidActionResult} holding the empty container if the fluid
	 *         handler was filled. NOTE If the container is consumable, the empty
	 ...
	 * @param world       World to play the sound in. Null for no sound.
	 * @param pos         BlockPos to play the sound at. Null for no sound.
	 * @return A {@link FluidActionResult} holding the filled container if
	 *         successful.
	 */
	@Nonnull
	...
	/**
	 * Fill a destination fluid handler from a source fluid handler with a max
	 * amount. To transfer as much as possible, use {@link Integer#MAX_VALUE} for
	 * maxAmount.
	 *
	 ...
	 * @param maxAmount        The largest amount of fluid that should be
	 *                         transferred.
	 * @param doTransfer       True if the transfer should actually be done, false
	 *                         if it should be simulated.
	 * @return the fluidStack that was transferred from the source to the
	 *         destination. null on failure.
	 */
	@Nullable
	...
	/**
	 * Internal method for filling a destination fluid handler from a source fluid
	 * handler using a specific fluid. Assumes that "drainable" can be drained from
	 * "fluidSource". or
	 * {@link #tryFluidTransfer(IFluidHandler, IFluidHandler, int, boolean)}.
	 */
	@Nullable
