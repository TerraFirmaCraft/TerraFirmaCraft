...
public final class Helpers
{
		...
		} else
		{
			...
			for (Plant plant : TFCRegistries.PLANTS.getValuesCollection())
			{
				if (plant.getPlantType() == Plant.PlantType.SHORT_GRASS && rand.nextFloat() < 0.5f)
				{
					...
					if (world.isAirBlock(pos.up()) && plant.isValidLocation(temp, ChunkDataTFC.getRainfall(world, pos.up()), Math.subtractExact(world.getLightFor(EnumSkyBlock.SKY, pos.up()), world.getSkylightSubtracted())) && plant.isValidGrowthTemp(temp) && rand.nextDouble() < plantBlock.getGrowthRate(world, pos.up()))
					{
	...
	/**
	 * Copy from Item#rayTrace Returns a RayTraceResult containing first found block
	 * in Players reach.
	 *
	...
	/**
	 * Copied from {@link net.minecraft.entity.Entity#rayTrace(double, float)} as it
	 * is client only
	 *
	...
	/**
	 * Simple method to spawn items in the world at a precise location, rather than
	...
	/**
	 * Method for hanging blocks to check if they can hang. 11/10 description. NOTE:
	 * where applicable, remember to still check if the blockstate allows for the
	 * specified direction!
	 *
	 * @param pos    position of the block that makes the check
	 * @param facing the direction the block is facing. This is the direction the
	 *               block should be pointing and the side it hangs ON, not the side
	 *               it sticks WITH. e.g: a sign facing north also hangs on the
	 *               north side of the support block
	...
	/**
	 * Primarily for use in placing checks. Determines a solid side for the block to
	 * attach to.
	 *
	 * @param pos             position of the block/space to be checked.
	 * @param possibleSides   a list/array of all sides the block can attach to.
	 * @param preferredFacing this facing is checked first. It can be invalid or
	 *                        null.
	 * @return Found facing or null is none is found. This is the direction the
	 *         block should be pointing and the side it stick TO, not the side it
	 *         sticks WITH.
	 */
	...
	/**
	 * This is meant to avoid Intellij's warnings about null fields that are
	 * injected to at runtime Use this for things
	 * like @ObjectHolder, @CapabilityInject, etc. AKA - The @Nullable is
	 * intentional. If it crashes your dev env, then fix your dev env, not this. :)
	 *
