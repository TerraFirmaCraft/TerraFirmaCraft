...
def generate(rm: ResourceManager):
    ...
    story.advancement('bronze_age', icon('tfc:metal/sword/bronze'), 'The Bronze Age', 'Enter the Bronze Age by smithing a bronze tool', 'copper_age', multiple_all(inventory_changed('#tfc:tools/bronze'), inventory_changed('#tfc:tools/bismuth_bronze'), inventory_changed('#tfc:tools/black_bronze')))
    ...
    story.advancement('steel_age', icon('tfc:metal/ingot/steel'), 'Industrialized', 'Make your first steel tool', 'blast_furnace', inventory_changed('#tfc:tools/steel'), frame='challenge')
    story.advancement('black_steel', icon('tfc:metal/ingot/black_steel'), 'Back in Black', 'Make your first black steel tool', 'steel_age', inventory_changed('#tfc:tools/black_steel'), frame='challenge')
    story.advancement('blue_steel', icon('tfc:metal/ingot/blue_steel'), 'Feeling Blue', 'Make your first blue steel tool', 'black_steel', inventory_changed('#tfc:tools/blue_steel'), frame='challenge')
    story.advancement('red_steel', icon('tfc:metal/ingot/red_steel'), 'Seeing Red', 'Make your first red steel tool', 'black_steel', inventory_changed('#tfc:tools/red_steel'), frame='challenge')
    ...
    story.advancement('quern', icon('tfc:quern'), 'The Grind', 'Craft a quern and a handstone', 'raw_stone', multiple_all(inventory_changed('tfc:handstone'), inventory_changed('tfc:quern')), requirements=[['quern'], ['handstone']])
    story.advancement('flux', icon('tfc:powder/flux'), 'In Flux', 'Grind some flux using a quern', 'quern', inventory_changed('#tfc:welding_flux'))
    ...
    story.advancement('shield', icon('tfc:metal/shield/copper'), 'Take Cover!', 'Craft a shield from a double sheet', 'sheet', inventory_changed('#c:tools/shield'))
    # Parented to saw
    ...
    story.advancement('scraped_hide', icon('tfc:large_scraped_hide'), 'Soak and Scrape', 'Soak a hide in limewater and then scrape it', 'barrel', inventory_changed('#tfc:scraped_hide'))
    ...
    story.advancement('glass_colors', icon('minecraft:red_stained_glass'), 'Stained Glass', 'Make all colors of stained glass blocks, as well as clear and tinted glass', 'basin_pour', multiple_all(inventory_changed('minecraft:glass'), inventory_changed('minecraft:tinted_glass'), *[inventory_changed('minecraft:%s_stained_glass' % c, name=c) for c in COLORS]), requirements=[['glass'], ['tinted_glass'], *[[c] for c in COLORS]], frame='challenge')
    ...
    story.advancement('blowpipe', icon('tfc:ceramic_blowpipe'), 'Blowpipe', 'Make a blowpipe', 'sheet', inventory_changed('#c:tools/blowpipe'))
    story.advancement('glassworking_tools', icon('tfc:ceramic_blowpipe'), 'Glassworking Tools', 'Make a paddle, jacks, a wool cloth, and a gem saw', 'blowpipe', multiple_all(*[inventory_changed('tfc:%s' % m, name=m) for m in ('paddle', 'jacks', 'wool_cloth', 'gem_saw')]), requirements=[[m] for m in ('paddle', 'jacks', 'wool_cloth', 'gem_saw')], frame='challenge')
    ...
    story.advancement('lens', icon('tfc:lens'), 'Optician', 'Use a lens to craft a daylight detector, spyglass, and a compass', 'blowpipe', multiple_all(*[inventory_changed('minecraft:compass'), inventory_changed('minecraft:daylight_detector'), inventory_changed('minecraft:spyglass')]), requirements=[['compass'], ['daylight_detector'], ['spyglass']])
    # Misc advancements
    ...
    story.advancement('iron_armor', icon('tfc:metal/chestplate/wrought_iron'), 'Knight in Shining Armor', 'Create a full set of wrought iron armor, a sword, and a shield', 'iron_age', multiple_all(inventory_changed('tfc:metal/sword/wrought_iron'), inventory_changed('tfc:metal/shield/wrought_iron'), *[inventory_changed('tfc:metal/%s/wrought_iron' % piece) for piece in TFC_ARMOR_SECTIONS]), requirements=[['metal/%s/wrought_iron' % item] for item in ('chestplate', 'helmet', 'greaves', 'boots', 'sword', 'shield')])
    ...
    world.advancement('volcano', icon('tfc:rock/magma/basalt'), 'Pacific Rim', 'Find an area with high volcanic activity', 'root', multiple_all(*[biome(b) for b in TFC_BIOMES if 'volcanic' in b]))
    ...
    world.advancement('globe_trotter', icon('minecraft:map'), 'Globe Trotter', 'Travel to positive 10,000 and -10,000 z, the hottest and coldest points nearest to spawn', 'root', multiple_all(location({'position': {'z': {'min': 9999}}}, name='high'), location({'position': {'z': {'max': -9999}}}, name='low')), requirements=[['high'], ['low']], frame='challenge')
    ...
    world.advancement('nickel', icon('tfc:ore/small_garnierite'), 'Nickels and Dimes', 'Find Garnierite', 'nugget', multiple_all(*[inventory_changed('tfc:ore/%s_garnierite' % o, name=o) for o in ('small', 'poor', 'normal', 'rich')]))
    world.advancement('iron', icon('tfc:ore/small_hematite'), 'Pretty Ironic', 'Find Hematite, Limonite, and Magnetite nuggets', 'nugget', multiple_all(*[inventory_changed('tfc:ore/small_%s' % o, name=o) for o in ('hematite', 'limonite', 'magnetite')]), requirements=[[o] for o in ('hematite', 'limonite', 'magnetite')])
    ...
    world.advancement('minerologist', icon('tfc:ore/halite'), 'Minerologist', 'Find every non-metal mineral in TFC', 'gemologist', multiple_all(*[inventory_changed('tfc:ore/%s' % g, name=g) for g in ALL_MINERALS]), frame='goal', requirements=[[g] for g in ALL_MINERALS])
    world.advancement('metallurgist', icon('tfc:metal/ingot/gold'), 'Metallurgist', 'Obtain an ingot for every metal in TFC', 'nugget', multiple_all(*[inventory_changed('tfc:metal/ingot/%s' % m, name=m) for m in METALS.keys()]), requirements=[[m] for m in METALS], frame='goal')
...
def multiple_all(*conditions: Json) -> Json:
    merged = {}
    for c in conditions:
        merged.update(c)
    return merged
