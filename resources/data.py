...
def generate(rm: ResourceManager):
    ...
    for category in ROCK_CATEGORIES:  # Rock (Category) Tools
        for tool in ROCK_CATEGORY_ITEMS:
            ...
            rm.item_tag("goes_on_tool_rack", 'tfc:stone/%s/%s' % (tool, category))
    ...
    for metal, metal_data in METALS.items():  # Metal Tools
        if 'tool' in metal_data.types:
            for tool_type, tool_tag in TOOL_TAGS.items():
                ...
                rm.item_tag("goes_on_tool_rack", 'tfc:metal/%s/%s' % (tool_type, metal))
            rm.item_tag("goes_on_tool_rack", 'tfc:metal/fishing_rod/%s' % metal, 'tfc:metal/tuyere/%s' % metal)
    ...
    rm.item_tag('goes_on_tool_rack', 'tfc:firestarter', 'minecraft:bow', 'minecraft:crossbow', 'minecraft:flint_and_steel')
