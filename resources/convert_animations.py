...
code = "//Animation transubstantiated by Troggacompany\n\n"
code = code + "public static final Animation %s = new Animation.Builder(%s)\n" % (name, str(duration) + "F")

for i in bones:
    #a_name is the name of the part moving
    a_name = i
    #We're assuming the animation is linear. You can change this if you want I guess.
    code = code + ".bone(\"%s\", new Animation.Bone.Builder(Easing.LINEAR)" % a_name
    ...
    for time in times:
        ...
        #Rotation parameters (AN: use quaternions :D)
        params = times[time]
        #Even deals with null rotation!
